...
| expandedImagesFilter                  | Adds `expandImages=true` to the request query if the INCLUDE_RICH_CONTENT, INTERNAL_UNSTABLE and EXPAND_RICH_CONTENT policies are present                                                                          | /content-preview, /internalcontent-preview, /enrichedcontent, /internalcontent                   |
...
| EXPAND_RICH_CONTENT        | If present along with INCLUDE_RICH_CONTENT and INTERNAL_UNSTABLE, it allows expanding rich content related fields in the response | mainImage, embeds, alternativeImages, promotionalImage, members, leadImages, image                           |
